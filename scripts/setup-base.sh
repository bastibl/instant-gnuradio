...
sudo apt-get -y install clang cmake cmake-qt-gui curl git git-core git-gui gitk htop libboost-all-dev open-vm-tools-desktop python-apt python-pip screen silversearcher-ag terminator tig tmux virtualbox-guest-dkms virtualbox-guest-utils virtualbox-guest-x11 wget xterm xvfb
